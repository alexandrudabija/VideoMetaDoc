...
export class FiltersComponent implements OnInit ,OnDestroy{
  ...
  setPriceMin: number | undefined;
  setPriceMax: number | undefined;
  ...
  ngOnInit(): void {
    ...
    this.dataManufacturesCategories$
      ...
      .subscribe((data: Products) => {
        ...
        this.setPriceMax = Math.max(...data.products.map((product) => product.price));

        this.setPriceMin = Math.min(...data.products.map((product) => product.price));
...
// the resulting arrays are assigned to the manufacturers and categories properties of the component.
  ...
  onShowManufacturer(): void {
    if (this.selectedManufacturer.length > 0) {
      this.showManufacturer.emit(this.selectedManufacturer);


    } else
    {
      this.showManufacturer.emit([]);
